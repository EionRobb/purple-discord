...
typedef enum {
	...
	PERM_VIEW_CREATOR_MONETIZATION_ANALYTICS = 0x20000000000, //1 << 41
	PERM_USE_SOUNDBOARD = 0x40000000000,            //1 << 42
	PERM_CREATE_GUILD_EXPRESSIONS = 0x80000000000,  //1 << 43
	PERM_CREATE_EVENTS = 0x100000000000,            //1 << 44
	PERM_USE_EXTERNAL_SOUNDS = 0x200000000000,      //1 << 45
	PERM_SEND_VOICE_MESSAGES = 0x400000000000,      //1 << 46
	PERM_USE_CLYDE_AI = 0x800000000000,             //1 << 47
	PERM_SET_VOICE_CHANNEL_STATUS = 0x1000000000000,//1 << 48
	PERM_SEND_POLLS = 0x2000000000000,              //1 << 49
	PERM_USE_EXTERNAL_APPS = 0x4000000000000,       //1 << 50
	PERM_PIN_MESSAGES = 0x8000000000000,            //1 << 51
...
discord_get_conv_from_channel_id(DiscordAccount *da, guint64 channel_id)
{
...
discord_get_react_text(DiscordAccount *da, const gchar *author_nick, const gchar *reactors_text, DiscordReaction *react)
{
	...
	PurpleConversation *conv = discord_get_conv_from_channel_id(da, react->room_id);
...
discord_reactor_cb(DiscordAccount *da, JsonNode *node, gpointer user_data)
{
	...
	PurpleConversation *conv;
	conv = discord_get_conv_from_channel_id(da, room_id);
	...
	if (channel_id_s && g_hash_table_contains(da->one_to_ones, channel_id_s)) {
		...
		if (reactions != NULL && purple_account_get_bool(da->account, "show-reactions", TRUE)) {
			...
			for (guint n = 0; n < reactions_len; n++) {
				...
				reaction_data->room_id = channel_id;
...
discord_process_dispatch(DiscordAccount *da, const gchar *type, JsonObject *data)
{
	if (purple_strequal(type, "PRESENCE_UPDATE")) {
		...
		PurpleConversation *conv;
		conv = discord_get_conv_from_channel_id(da, to_int(channel_id_s));
		if (conv == NULL) {
			g_free(msg_times);
			return;
		}
...
discord_react_cb(DiscordAccount *da, JsonNode *node, gpointer user_data)
{
	...
	PurpleConversation *conv = discord_get_conv_from_channel_id(da, to_int(channel_id_s));

	gchar *react_text = discord_get_react_text(da, author_nick, reactor_nick, react);
	g_free(author_nick);
	g_free(reactor_nick);

	purple_conversation_write(conv, NULL, react_text, PURPLE_MESSAGE_SYSTEM, react->msg_time);
...
discord_reply_cb(DiscordAccount *da, JsonNode *node, gpointer user_data)
{
	...
	PurpleConversation *conv = discord_get_conv_from_channel_id(da, room_id);
	DiscordGuild *guild = NULL;
	DiscordChannel *channel = discord_get_channel_global_int_guild(da, room_id, &guild);
	...
	if (guild) {
		gchar *name = discord_create_nickname_from_id(da, guild, channel, da->self_user_id);
		purple_serv_got_chat_in(da->pc, discord_chat_hash(room_id), name, PURPLE_MESSAGE_SEND, msg_txt, time(NULL));
		g_free(name);
	}
